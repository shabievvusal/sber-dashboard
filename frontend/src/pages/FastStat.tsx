...
export default function FastStat() {
  ...
  const [expandedIdleTimes, setExpandedIdleTimes] = useState<Set<number>>(new Set());
  ...
  function processApiTasks(tasks: ApiTask[]): Employee[] {
    ...
    tasks.forEach((task) => {
      ...
      const mx = task.mx || '';
      const warehouseOrder = task.warehouseOrder || '';
  ...
  async function loadDayData(date: string, retryCount: number = 0) {
    if (!date) return;
    ...
    try {
      ...
      // Проверяем статус обработки (202 Accepted)
      if (response.status === 202 || response.data.status === 'processing') {
        const maxRetries = 30; // Максимум 30 попыток (около 30 секунд)
        if (retryCount < maxRetries) {
          // Показываем сообщение о обработке
          setError('Данные обрабатываются, пожалуйста, подождите...');
          // Ждем 1 секунду и повторяем запрос
          setTimeout(() => {
            loadDayData(date, retryCount + 1);
          }, 1000);
          return;
        } else {
          setError('Превышено время ожидания обработки данных. Попробуйте обновить страницу.');
          setLoading(false);
          return;
        }
      }
      ...
      // Обрабатываем статус 202 из ошибки
      if (error.response?.status === 202 || error.response?.data?.status === 'processing') {
        const maxRetries = 30;
        if (retryCount < maxRetries) {
          setError('Данные обрабатываются, пожалуйста, подождите...');
          setTimeout(() => {
  ...
  async function captureScreenshot(companyName?: string) {
    ...
    try {
      ...
      const topLeadersForScreenshot = dataToScreenshot
      ...
      dataToScreenshot.forEach((emp, index) => {
  ...
  // Получаем общий топ-3 лидеров по количеству задач для кубков
  const topLeaders = useMemo(() => {
    const allSorted = [...filteredData].sort((a, b) => {
      if (b.taskCount !== a.taskCount) return b.taskCount - a.taskCount;
      return b.totalWeight - a.totalWeight;
    });
    return allSorted.slice(0, 3).map(e => e.employee);
  }, [filteredData]);
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-5">
        ...
        <div className="bg-white rounded-lg shadow-lg p-8 mb-8">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-3xl font-bold text-gray-800 border-b-4 border-blue-500 pb-2">
               Анализ работы сотрудников - КДК и ХР
            </h1>
            <LogoutButton />
          </div>
            ...
            <div>
              ...
              <div className="overflow-x-auto mb-6" ref={tableRef}>
                <table className="w-full border-collapse shadow-lg">
                  <thead>
                    <tr>
                      <th className="bg-blue-500 text-white p-3 text-center font-bold sticky top-0" style={{ width: '80px' }}>
                        №
                      </th>
                      <th
                  ...
                  <tbody>
                    {pageData.map((emp, index) => {
                      ...
                      const globalIndex = topLeaders.indexOf(emp.employee);
                      const trophyPosition = globalIndex === 0 ? 'gold' : globalIndex === 1 ? 'silver' : globalIndex === 2 ? 'bronze' : null;
                      // Номер в таблице (с учетом пагинации)
                      ...
                      return (
                        ...
                        >
                          <td className="p-3 border-b border-gray-200 text-center" style={{ position: 'relative', width: '100px' }}>
                            {/* Кубок для топ-3 - абсолютно позиционирован слева от номера */}
                            {trophyPosition && (
                              <div 
                                className={`absolute ${trophyPosition === 'gold' ? 'animate-pulse' : ''}`}
                              ...
                              >
                                {trophyPosition === 'gold' && <span className="text-2xl">🥇</span>}
                                {trophyPosition === 'silver' && <span className="text-2xl">🥈</span>}
                                {trophyPosition === 'bronze' && <span className="text-2xl">🥉</span>}
                              </div>
                            )}
                            {/* Номер - всегда по центру */}
                            <div className="inline-flex px-2 py-1 rounded bg-gradient-to-br from-blue-600 to-blue-700 border-2 border-blue-300 shadow-md">
                              <span className="text-sm font-extrabold text-white" style={{ textShadow: '0 1px 2px rgba(0,0,0,0.5)' }}>
                                #{rowNumber}
                              </span>
                            </div>
                          </td>
      ...
      {showModal && selectedEmployee && (
        ...
        >
          <div
            className={`bg-white m-8 p-8 rounded-lg max-w-6xl mx-auto relative overflow-y-auto ${
              currentModalTab === 'idle' ? 'max-h-[98vh]' : 'max-h-[95vh]'
            }`}
              ...
              <div className="bg-gray-50 p-5 rounded-lg">
                ...
                <div className="max-h-[75vh] overflow-y-auto space-y-2">
                  {selectedEmployee.tasks.map((task, index) => {
                    ...
                    // Для задач ХР и КДК товар отображается сразу после значка
                    const isHR = task.processType === '2060';
                    const isKDK = task.processType === '2021';

                    // Определяем смену заказа для подсветки
                    const prevTask = index > 0 ? selectedEmployee.tasks[index - 1] : null;
                    const nextTask = index < selectedEmployee.tasks.length - 1 ? selectedEmployee.tasks[index + 1] : null;
                    
                    const isLastInOrder = nextTask && 
                      task.warehouseOrder && 
                      nextTask.warehouseOrder && 
                      task.warehouseOrder !== nextTask.warehouseOrder;
                    
                    const isFirstInOrder = prevTask && 
                      task.warehouseOrder && 
                      prevTask.warehouseOrder && 
                      task.warehouseOrder !== prevTask.warehouseOrder;

                    // Определяем класс для подсветки
                    let bgColorClass = 'bg-white';
                    if (isLastInOrder) {
                      bgColorClass = 'bg-red-50'; // Легкий красный для последней задачи в заказе
                    } else if (isFirstInOrder) {
                      bgColorClass = 'bg-green-50'; // Легкий зеленый для первой задачи в новом заказе
                    }

                    return (
                      <div key={index} className={`${bgColorClass} p-4 rounded border-l-3 border-blue-500 relative`}>
                        <div className="flex justify-between items-center mb-2 pb-2 border-b">
                          <span className="font-bold">
                            {task.time} {processBadge}
                            {(isHR || isKDK) && task.product && (
                              <span className="ml-2"> {task.product}</span>
                            )}
                          </span>
                        ...
                        {task.mx && (
                          <div className="absolute right-4 top-16 text-sm font-bold text-gray-600">
                            МХ: {task.mx}
                          </div>
                        )}
                      </div>
                ...
                <div className="space-y-2">
                  {selectedEmployee.idleTimes.length > 0 ? (
                    selectedEmployee.idleTimes.map((idle, idx) => {
                      const isExpanded = expandedIdleTimes.has(idx);
                      
                      // Находим задачи до и после простоя
                      // idle.from - время последней задачи перед простоем
                      // idle.to - время первой задачи после простоя
                      const sortedTasks = [...selectedEmployee.tasks].sort((a, b) => compareTime(a.time, b.time));
                      const beforeTaskIndex = sortedTasks.findIndex(task => task.time === idle.from);
                      const afterTaskIndex = sortedTasks.findIndex(task => task.time === idle.to);
                      
                      const tasksToShow: Task[] = [];
                      if (beforeTaskIndex >= 0 && afterTaskIndex >= 0) {
                        // Показываем 2 задачи: задачу перед простоем и задачу после простоя
                        // Задача перед простоем
                        if (beforeTaskIndex >= 0 && beforeTaskIndex < sortedTasks.length) {
                          tasksToShow.push(sortedTasks[beforeTaskIndex]);
                        }
                        // Задача после простоя
                        if (afterTaskIndex >= 0 && afterTaskIndex < sortedTasks.length) {
                          tasksToShow.push(sortedTasks[afterTaskIndex]);
                        }
                      }
                      ...
                      return (
                        <div key={idx}>
                          <div 
                            className="bg-yellow-50 p-3 rounded border-l-4 border-yellow-400 cursor-pointer hover:bg-yellow-100 transition"
                            onClick={() => {
                              const newSet = new Set(expandedIdleTimes);
                              if (isExpanded) {
                                newSet.delete(idx);
                              } else {
                                newSet.add(idx);
                              }
                              setExpandedIdleTimes(newSet);
                            }}
                          >
                            <div className="flex justify-between items-center">
                              ...
                              <div className="flex items-center gap-2">
                                <span className="text-orange-600">{idle.formatted}</span>
                          ...
                          {isExpanded && tasksToShow.length > 0 && (
                            <div className="mt-2 space-y-2">
                              {tasksToShow.map((task, taskIdx) => {
                                ...
                                const isHR = task.processType === '2060';
                                const isKDK = task.processType === '2021';
                                const taskIndexInAll = selectedEmployee.tasks.findIndex(t => t.time === task.time && t.product === task.product);

                                // Определяем смену заказа для подсветки
                                const prevTask = taskIndexInAll > 0 ? selectedEmployee.tasks[taskIndexInAll - 1] : null;
                                const nextTask = taskIndexInAll >= 0 && taskIndexInAll < selectedEmployee.tasks.length - 1 ? selectedEmployee.tasks[taskIndexInAll + 1] : null;
                                ...
                                return (
                                  <div key={taskIdx} className={`${bgColorClass} p-4 rounded border-l-3 border-blue-500 relative ml-4`}>
                                    <div className="flex justify-between items-center mb-2 pb-2 border-b">
                                      <span className="font-bold">
                                        {task.time} {processBadge}
                                        {(isHR || isKDK) && task.product && (
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm">
                                      ...
                                      {taskIndexInAll > 0 && (
                                        <div className="col-span-4">
                                          <span className="font-bold text-gray-600">Время от предыдущей:</span>{' '}
                                          {formatTimeDifference(
                                            timeDifference(task.time, selectedEmployee.tasks[taskIndexInAll - 1].time)
                                          )}
                                        </div>
                                      )}
                                    </div>
